...
const Landing = () => {
  const { user, setLoginModalOpen, language } = useStore();
    ...
    <div className="flex flex-col" style={{ gap: '6rem', padding: '2rem 0', marginBottom: '4rem' }}>
      ...
      <section style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '3rem', minHeight: '70vh' }}>
        ...
        >
          <div className="badge badge-warning" style={{ marginBottom: '1.5rem', fontWeight: 'bold', letterSpacing: '0.1em', textTransform: 'uppercase' }}>
            <Zap size={14} className="inline mr-1" /> {t.heroBadge}
          </div>
          <h1 className="text-3xl font-extrabold leading-tight" style={{ marginBottom: '1.5rem', fontSize: 'clamp(2.5rem, 5vw, 4rem)', lineHeight: '1.1' }}>
            {t.heroTitle1}<span className="gradient-text">{t.heroTitle2}</span><br />
            {t.heroTitle3}<span style={{ color: 'var(--text-secondary)' }}>{t.heroTitle4}</span>
          </h1>
          ...
          <div className="flex gap-4">
            {!user ? (
              ...
              >
                {t.heroBtn} <ArrowRight size={20} className="ml-2" />
              </motion.button>
            ) : (
              <motion.button 
                whileHover={{ scale: 1.05 }}
                ...
                style={{ padding: '1rem 2rem', fontSize: '1.125rem' }}
              >
                {t.heroBtnDashboard} <ArrowRight size={20} className="ml-2" />
              </motion.button>
            )}
          </div>
        </motion.div>
        ...
        >
          ...
          >
            <div className="flex justify-between items-center mb-4">
              <span className="badge badge-success">{t.targetReached}</span>
              <span className="text-xs text-secondary font-bold">{t.liveUpdate}</span>
            </div>
            <h3 className="font-bold text-xl mb-2">{t.mockTitle}</h3>
            <div className="progress-bar mb-2">
              <div className="progress-fill" style={{ width: '100%' }}></div>
            </div>
            <p className="text-sm font-semibold" style={{ color: 'var(--success)' }}>{t.mockCollected}</p>
            ...
            <div className="flex items-center mt-4" style={{ marginTop: '1.5rem', gap: '0.5rem' }}>
              ...
              <span className="text-xs text-secondary font-medium" style={{ marginLeft: '0.5rem' }}>{t.mockDonors}</span>
            </div>
            ...
            >
              ...
              <div>
                <div className="text-sm font-bold">{t.mockSecure}</div>
      ...
      >
        <div style={{ marginBottom: '3rem' }}>
          <h2 className="text-3xl font-extrabold mb-4">{t.bentoTitle1}<span className="text-secondary">{t.bentoTitle2}</span></h2>
          <p className="text-secondary text-lg" style={{ maxWidth: '42rem' }}>{t.bentoDesc}</p>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1.5rem', gridAutoRows: 'minmax(200px, auto)' }}>
          ...
          <motion.div variants={itemVariants} className="card flex flex-col justify-between" style={{ gridColumn: '1 / -1', background: 'var(--accent-gradient)', color: 'white' }}>
            <div style={{ marginBottom: '2rem' }}>
              ...
              <h3 className="text-2xl font-bold mb-2">{t.feature1Title}</h3>
              <p style={{ opacity: 0.9, maxWidth: '36rem' }}>{t.feature1Desc}</p>
            </div>
          </motion.div>
          ...
          <motion.div variants={itemVariants} className="card flex flex-col" style={{ gap: '1rem' }}>
            ...
            <p className="text-secondary">{t.feature2Desc}</p>
          </motion.div>
          ...
          <motion.div variants={itemVariants} className="card flex flex-col" style={{ gap: '1rem' }}>
            ...
            <p className="text-secondary">{t.feature3Desc}</p>
          </motion.div>
      ...
      >
        ...
        <div style={{ position: 'relative', zIndex: 1 }}>
          ...
          <p className="text-secondary text-lg mx-auto" style={{ marginBottom: '2rem', maxWidth: '36rem' }}>{t.ctaDesc}</p>
          {!user ? (
